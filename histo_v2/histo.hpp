//
// Created by iotuser on 22. 4. 26.
//

#ifndef HISTO_V2_HISTO_HPP
#define HISTO_V2_HISTO_HPP

#endif //HISTO_V2_HISTO_HPP
...
void calc_histo1(const Mat& img, Mat& hist, Vec3i bins, Vec3f range, int _dims)
{
...
vector<Mat> load_histo1(Vec3i bins, Vec3f ranges, int nImages)
{
    vector<Mat> DB_hists;
    for (int i = 0; i < nImages; i++)
    {
        String  fname = format("/home/iotuser/다운로드/DB/img_%02d.jpg", i);
        Mat hsv, hist, img = imread(fname, IMREAD_COLOR);
        if (img.empty()) continue;

        cvtColor(img, hsv, COLOR_BGR2HSV);			// HSV 컬러 변환
        calc_histo1(hsv, hist, bins, ranges, 2);			// 2차원 히스토그램 계산
        DB_hists.push_back(hist);
    }
    cout << format("%d 개의 파일을 로드 및 히스토그램 계산 완료", (int)DB_hists.size()) << endl;
    return DB_hists;
}
