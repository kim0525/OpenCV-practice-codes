...
#ifndef CLASSIFY_CARNUMBER_CLASSIFY_OBJECTS_HPP
#define CLASSIFY_CARNUMBER_CLASSIFY_OBJECTS_HPP

#endif //CLASSIFY_CARNUMBER_CLASSIFY_OBJECTS_HPP
...
void classify_numbers(vector<Mat> numbers, Ptr<ml::KNearest> knn[2], int K1, int K2)
{
    ...
    for (int i = 0; i < (int)numbers.size(); i++) {
        ...
        if (i == 2) {
            ...
            //test01[i] = text_value[(int)results.at<float>(0)];
            test02 += text_value[(int) results.at<float>(0)];
            ...
            //test01[i] = to_string((int)results.at<float>(0));
            test02 += to_string((int) results.at<float>(0));
    ...
    cout << endl<< "test02 출력: " << test02 <<endl;
}